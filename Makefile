# Всем привет!
# Меня зовут Михаил Сокольский
# https://t.me/timeforpeople
# https://github.com/mysokolsky

# Это многофункциональный Makefile, который позволяет запускать проекты заданий несколькими способами:

# 1)
# make 
# - запустит последнее сделанное задание. 
# Если последнее сделанное задание в папке L2.5, то запустит именно его
# Если в папке последнего задания нет файла main.go, то автоматически создаст (ниже в цели run:) и запустит его 

# 2)
# make L2.3
# - запустит задание в папке L2.3

# 3)
# make L1.8 <<< "-010011 1 0"
# - запустит задание в папке L1.8 с параметрами, передаваемыми в запущенную программу
# Равносильно строке кода:
# cd L1.8; go run . <<< "-010011 1 0"; cd ..

# 4)
# make push
# - запустит на выполнение цель push: из данного мейкфайла
